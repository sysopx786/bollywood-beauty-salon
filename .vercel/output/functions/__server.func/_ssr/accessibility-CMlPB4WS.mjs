import { o as PHONE_DISPLAY, s as PHONE_TEL } from "./salon-C_Hv5RvQ.mjs";
import { x as require_jsx_runtime } from "../_libs/@tanstack/react-router+[...].mjs";
import { a as PageIntro } from "./router-C0-eDx_d.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/accessibility-CMlPB4WS.js
var import_jsx_runtime = require_jsx_runtime();
function AccessibilityPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PageIntro, {
		kicker: "Accessibility",
		title: "The site should be readable. The building details are a phone call.",
		lede: "Pages use text instead of prices trapped in photographs, visible keyboard focus, and a skip link. Motion is reduced when your device asks for that."
	}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "mx-auto grid max-w-3xl gap-4 px-5 pb-20 text-ink/80",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
				"If a page is hard to use, call",
				" ",
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("a", {
					href: `tel:${PHONE_TEL}`,
					className: "text-wine",
					children: PHONE_DISPLAY
				}),
				" ",
				"and say what you were trying to do. There is no separate accessibility inbox on this site."
			] }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "The salon has not published parking, step-free entrance, or restroom details. If access matters to the trip, ask on the phone before you come. This website will not guess." }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Sunday’s closing time is also best confirmed by phone. The site says the salon opens at 10:00 AM and does not pick a closing time the listings disagree on." })
		]
	})] });
}
//#endregion
export { AccessibilityPage as component };
